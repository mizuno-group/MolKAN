...
parser.add_argument('--num_grids', type=utils.parse_list_or_int, default=16, help="num_grids (int or list(int))")
parser.add_argument("--dropout", type=utils.parse_list_or_float, default=0.0, help="dropout probability (float or list(float))")
...
if isinstance(cfg["lr"], float):
    cfg["lr"] = [cfg["lr"]]
if isinstance(cfg["lambda"], float):
    cfg["lambda"] = [cfg["lambda"]]
if isinstance(cfg["dropout"], float):
    cfg["dropout"] = [cfg["dropout"]]
...
def prepare_model(logger, num_grids, lr, l, p):
    """
    ...
    if cfg["layer"] == 1:
        model = FourierKAN_Predictor([512, 1], cfg["mode"], num_grids, p) # change this to your model
    elif cfg["layer"] == 2:
        model = FourierKAN_Predictor([512, 512, 1], cfg["mode"], num_grids, p)
    elif cfg["layer"] == 3:
        model = FourierKAN_Predictor([512, 512, 512, 1], cfg["mode"], num_grids, p)
    else:
...
def main():
    if args.train:
        ...
        valid_metrics_all = {}
        test_scores_all = {}
        for g in cfg["num_grids"]:
            for i, (train_loader, valid_loader, test_loader) in enumerate(zip(train_loaders, valid_loaders, test_loaders)):
                for lr in cfg["lr"]:
                    for p in cfg["dropout"]:
                        for l in cfg["lambda"]:
                            logger.info(f" ===== experiments with grids: {g}, dropout: {p}, batch size: {cfg["batch_size"][i]}, lr: {lr}, lambda: {l} ===== ")
                            start = time.time() # for time stamp
                            model, loss_func, optimizer, scheduler = prepare_model(logger, g, lr, l, p)
                            exp_note = f"grids_{g}_dropout_{p}_bs_{cfg["batch_size"][i]}_lr_{lr}_lambda_{l}"
                            ...
                            logger.info(f" ===== experiments with grids: {g}, dropout: {p}, batch size: {cfg["batch_size"][i]}, lr: {lr}, lambda: {l} finished ===== ")
        ...
        for idx, sc in enumerate(plot_scores):
            ...
            axes = fig.subplots(1, len(cfg["num_grids"]))
            ...
            for i in range(len(cfg["num_grids"])):
                data = np.zeros((len(cfg["batch_size"]), len(cfg["dropout"])))
                for j in range(len(cfg["batch_size"])):
                    for k in range(len(cfg["dropout"])):
                        data[j, k] = test_scores_all[f"grids_{cfg["num_grids"][i]}_dropout_{cfg["dropout"][k]}_bs_{cfg["batch_size"][j]}_lr_0.0005_lambda_0.0"][idx]
                sns.heatmap(data, annot=True, fmt=".3f", xticklabels=cfg["dropout"], yticklabels=cfg["batch_size"], ax=axes[i], cmap="Blues", cbar=False, vmin=vmin, vmax=vmax)
                axes[i].set_title(f"grids: {cfg["num_grids"][i]}", fontsize=18)

            plt.suptitle(f"{cfg["note"]} {sc}", fontsize=20, c="red")
            axes[0].set_xlabel("dropout", fontsize=14)
            axes[0].set_ylabel("batch_size", fontsize=14)
            cbar_ax = fig.add_axes([0.92, 0.15, 0.02, 0.7])
            plt.colorbar(axes[len(cfg["num_grids"])-1].collections[0], cax=cbar_ax)
