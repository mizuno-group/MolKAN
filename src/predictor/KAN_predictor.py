...
class KAN_predictor(nn.Module):
    def __init__(self, width:list, device, working_dir:Path, results_dir_name:str, LOGGER):
        ...
        self.model = KAN(width=width, grid=5, k=3, auto_save=False, device=device, seed=42)
        self.width = width
        self.device = device
    ...
    def train(self, tr_loader, val_loader, lr, lamb,
        ...
        # init model
        self.model = KAN(width=self.width, grid=5, k=3, device=self.device, seed=42, auto_save=False)
        ...
        for _ in pbar:
            ...
            for x, y in tr_loader:
                ...
                for f in self.metrics:
                    if f.__name__ == "roc_auc_score" or f.__name__ == "r2_score" or f.__name__ == "root_mean_squared_error":
                        ...
                        tr_metrics_batch[f.__name__].append(f(y.cpu().detach().numpy(), np.round(np.clip(pred.cpu().detach().numpy(), 0, 1))))
                    else:
                        tr_metrics_batch[f.__name__].append(f(y.cpu().detach().numpy(), np.round(np.clip(pred.cpu().detach().numpy(), 0, 1)), zero_division=0.0))
            ...
            for f in self.metrics:
                ...
                if f.__name__ == "roc_auc_score" or f.__name__ == "r2_score" or f.__name__ == "root_mean_squared_error":
                    ...
                    results["val_metrics"][f.__name__].append(f(val_label.cpu().detach().numpy(), np.round(np.clip(val_pred.cpu().detach().numpy(), 0, 1))))
                else:
                    results["val_metrics"][f.__name__].append(f(val_label.cpu().detach().numpy(), np.round(np.clip(val_pred.cpu().detach().numpy(), 0, 1)), zero_division=0.0))
    ...
    def optimize(self, trial_num, x_tr_val:np.ndarray, y_tr_val:np.ndarray, mode, fold, batch_size, epochs, patience):
    ...
    def test(self, x_test, y_test, lr=None, lamb=None):
        self.x_test = x_test
        self.y_test = y_test
        ...
        test_pred = (test_pred / self.fold).cpu().detach().numpy()
        test_results = {}
        for f in self.metrics:
            if f.__name__ == "roc_auc_score" or f.__name__ == "r2_score" or f.__name__ == "root_mean_squared_error":
                test_results[f.__name__] = f(self.y_test, test_pred)
            else:
                test_results[f.__name__] = f(self.y_test, np.round(np.clip(test_pred, 0, 1)))
    ...
    def test_with_grids_refine(self, x_test, y_test, lr=None, lamb=None):
        self.x_test = x_test
        self.y_test = y_test
        ...
        for i, (tr_idx, val_idx) in enumerate(self.kf.split(self.x_tr_val, self.y_tr_val)):
            ...
            if lr is None and lamb is None:
                ...
                self.model.refine(new_grid=grids[0])
                results2 = self.train(tr_loader, test_loader, **best_params, test_with_grids_refine=True, fold_idx=i)
                self.LOGGER.info(f"---grids refine 10 -> 20")
                self.model.refine(new_grid=grids[1])
                results3 = self.train(tr_loader, test_loader, **best_params, test_with_grids_refine=True, fold_idx=i)
            else:
                results = self.train(tr_loader, val_loader, lr=lr, lamb=lamb, test_with_grids_refine=True, fold_idx=i)
                self.LOGGER.info(f"---grids refine 5 -> 10")
                self.model.refine(new_grid=grids[0])
                results2 = self.train(tr_loader, test_loader, lr=lr, lamb=lamb, test_with_grids_refine=True, fold_idx=i)
                self.LOGGER.info(f"---grids refine 10 -> 20")
                self.model.refine(new_grid=grids[1])
                results3 = self.train(tr_loader, test_loader, lr=lr, lamb=lamb, test_with_grids_refine=True, fold_idx=i)
